...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        if (ret == p_view){
            ...
            printf ("--------------------------\n");
            Status ret1 = read_and_validate_mp3_file (argv, &mp3tagInfo);		// validate all the arguments passed in Command Line.
            if (ret1 == p_success){
                Status ret2 = view_tag (argv, &mp3tagInfo);						//execute the Viewing process of the MP3 Tag.
                if (ret2 == p_success)		
                {
                    printf ("INFO: Done.\n");
                    printf ("--------------------------\n");
